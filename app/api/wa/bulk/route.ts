...
export async function POST(req: Request) {
  ...
  const { customer_ids, channel, template_name } = body || {};
  ...
  // نجيب أرقام العملاء المختارين
  const { data: custs } = await admin.from("customers").select("id, phone1").in("id", customer_ids);
  const targets = (custs || []).filter((c: any) => c.phone1).map((c: any) => ({ id: c.id, num: normNum(c.phone1) })).filter((t) => t.num);
    ...
    await Promise.all(slice.map(async (t) => {
      try {
        ...
        const r = await fetch(apiUrl, {
          method: "POST", headers,
          body: JSON.stringify({ template_name, broadcast_name: template_name, parameters: [], channel_number: senderCh, channelNumber: senderCh, channelPhoneNumber: senderCh }),
        });
