...
export default function CustomerEdit({ customer, specialties }: { customer: C; specialties: Spec[] }) {
  ...
  // ربط زر الحفظ في الفوتر الثابت (DrawerFooter) عبر حدث عام — ref يمنع الـ stale closure
  ...
  async function save() {
    setErr(""); setMsg("");
    if (!f.name.trim()) { setErr("الاسم مطلوب"); window.dispatchEvent(new Event("niqat:customer-saved")); return; }
    setBusy(true);
  ...
  return (
    <>
      <AccordionSection title={tr("basicInfo")} defaultOpen>
        <div className="frow">
      ...
      <AccordionSection title={tr("salesInfo")}>
        <div className="frow">
