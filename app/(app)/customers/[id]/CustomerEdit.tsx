...
// يمنع العربي ويحوّل لإنجليزي كابيتال
function toEngUpper(v: string) {
  return v.replace(/[\u0600-\u06FF]/g, "").toUpperCase();
}
const hasArabic = (v: string) => /[\u0600-\u06FF]/.test(v);

const TabBtn = memo(function TabBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button type="button" onClick={onClick} className="tabbtn" data-active={active ? "1" : "0"}>
      {children}
    </button>
  );
});

function CustomerEdit({ customer, specialties, docsSection }: { customer: C; specialties: Spec[]; docsSection?: ReactNode }) {
  ...
  const save = useCallback(async () => {
    setErr(""); setMsg("");
    if (!f.name.trim()) { setErr("الاسم مطلوب"); setTab("basic"); return; }
    if (hasArabic(f.name)) { setErr("اسم العميل لازم يكون إنجليزي فقط."); setTab("basic"); return; }
    setBusy(true);
  ...
  return (
    <div className="card" style={{ padding: 18, marginBottom: 14 }}>
      <div className="tabs-row">
        <TabBtn active={tab === "basic"} onClick={() => setTab("basic")}>أساسية</TabBtn>
        <TabBtn active={tab === "sales"} onClick={() => setTab("sales")}>مبيعات</TabBtn>
        <TabBtn active={tab === "docs"} onClick={() => setTab("docs")}>مستندات</TabBtn>
      </div>
      ...
      <div className="tab-pane">
        ...
        {tab === "docs" && (
          <div>{docsSection || <div style={{ fontSize: 13, color: "var(--muted)" }}>لا يوجد قسم مستندات.</div>}</div>
        )}
      </div>
