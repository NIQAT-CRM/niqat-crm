...
export default function SubscriptionsPanel({
  ...
  async function doMove(e: Enr) {
    ...
    await logAudit("batch_transfer", `${e.diploma}: ${tr("movedFrom")} ${e.batch} ${tr("toWord")} ${batchLabel(moveTo)}`);
    setBusy(false); setMoveFor(null); setMoveTo("");
    toast(tr("moved")); router.refresh();
  }

  // إضافة دبلومة/اشتراك جديد
  async function addDiploma() {
    if (!nd.dip) { toast(tr("chooseDiploma")); return; }
    setBusy(true);
    ...
    const dipName = dipOpts.find((d) => d.v === nd.dip)?.label || tr("diplomaWord");
    await logAudit("enrollment_add", `${tr("addSubscriptionAudit")}: ${dipName}${nd.batch ? " — " + batchLabel(nd.batch) : ""}`);
    setBusy(false); setAdding(false); setNd({ dip: "", batch: "", amount: "", currency: "EGP" });
    toast(tr("diplomaAdded")); router.refresh();
  }

  return (
    <div className="card" style={{ padding: 18, marginBottom: 14 }}>
      <div className="sec-t" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span>{tr("subscriptionsLong")}</span>
        ...
        <div style={{ border: "1px solid var(--line)", background: "var(--surface)", borderRadius: 10, padding: 12, marginBottom: 10, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          {canFinance && (
            <div style={{ display: "flex", gap: 8 }}>
              <input className="inp num" dir="ltr" placeholder={tr("agreedAmount")} style={{ flex: 1 }} value={nd.amount} onChange={(e) => setNd((s) => ({ ...s, amount: e.target.value }))} />
