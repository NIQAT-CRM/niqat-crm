...
export default async function NewCustomerPage() {
  ...
  const [{ data: specs }, { data: dips }, { data: bts }, { data: affRow }] = await Promise.all([
    ...
    supabase.from("batches").select("id,code,price,currency,price_egp,price_usd,diploma_id").order("start_date", { ascending: false }),
  ...
  return (
    <div style={{ maxWidth: 620 }}>
      ...
      <NewCustomerForm
        ...
        batches={(bts || []).map((b) => ({ id: b.id, name: b.code, price: Number((b as any).price) || 0, currency: (b as any).currency || "EGP", price_egp: Number((b as any).price_egp) || 0, price_usd: Number((b as any).price_usd) || 0, diploma_id: (b as any).diploma_id || "" }))}
        meId={user?.id || ""}
