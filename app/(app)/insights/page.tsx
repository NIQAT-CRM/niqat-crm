...
export default async function InsightsPage() {
  ...
  const { data: profile } = await supabase.from("profiles").select("can_use_ai, ai_options, can_see_finance").eq("id", user.id).maybeSingle();
  ...
  // أوبشنز الذكاء الاصطناعي لكل مستخدم — كلها مقفولة افتراضياً، الأدمن بيفتح اللي عايزه
  const opt = (profile?.ai_options && typeof profile.ai_options === "object") ? (profile.ai_options as Record<string, boolean>) : {};
  const showTop = !!opt.top_diplomas;
  const showPeak = !!opt.peak_hours;
  const showBatches = !!opt.batches_filling;
  const showStale = !!opt.stale_leads;
  const showTrend = !!opt.collection_trend;
  ...
  const showNow = showTop || showPeak || (showTrend && !!trend);
  const showSug = showBatches || showStale || (showTrend && !!hasCollectionSuggestion);
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto" }}>
      ...
      {/* اللي بيحصل دلوقتي */}
      {showNow && <div className="ai-sh"><span className="tick" /><h2>{t("insightsNow")}</h2></div>}
      <div style={{ display: "grid", gap: 14, gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", alignItems: "stretch" }}>
        {/* أكتر دبلومة حجزاً */}
        {showTop && <div className="card ai-ins">
        ...
        {/* وقت الذروة */}
        {showPeak && <div className="card ai-ins">
        ...
        {/* اتجاه التحصيل — يظهر فقط لو الأوبشن مفتوح و trend مش null */}
        {showTrend && trend && (
      ...
      {/* اقتراحات ليك */}
      {showSug && <div className="ai-sh"><span className="tick" /><h2>{t("insSuggestions")}</h2></div>}
      {showSug && <div className="card">
        {/* باتشات قرب تكمل */}
        {showBatches && (batches.length > 0 ? batches.slice(0, 4).map((b) => (
        ...
        {/* التحصيل بيرتفع */}
        {showTrend && hasCollectionSuggestion && (
          <div className="ai-sug">
